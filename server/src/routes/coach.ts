...
router.get('/patients', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/patients/:id/logs', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/patients/:id/videos', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const id = req.params.id as string;
...
router.post('/sessions/:id/report', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) {
       res.status(401).json({ success: false, error: 'Unauthorized' });
...
router.get('/finances', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      res.status(401).json({ success: false, error: 'Unauthorized' });
...
router.post('/mock-session', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const userId = req.user?.id;
        ...
        if (!coachProfile) { res.status(404).json({ success: false, error: 'Coach profile not found' }); return; }

        const patientPackage = await prisma.patientPackage.findFirst({
            where: { patientProfileId, status: 'ACTIVE' }
        });

        if (!patientPackage) { res.status(404).json({ success: false, error: 'Patient package not found' }); return; }
