...
router.post('/prescriptions/:prescriptionId/submit', authenticateJWT, upload.single('video'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) {
       res.status(401).json({ success: false, error: 'Unauthorized' });
...
router.post('/checkin', authenticateJWT, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.id;
    if (!userId) {
       res.status(401).json({ success: false, error: 'Unauthorized' });
    ...
    const checkIn = await prisma.progressCheckIn.create({
      data: {
        ...
        painLevel: parseInt(painLevel as string, 10),
        painLocation: typeof painLocation === 'string' ? painLocation : null,
        notes: typeof notes === 'string' ? notes : null,
        sleepQuality: parseInt(sleepQuality as string, 10),
        energyLevel: parseInt(energyLevel as string, 10)
      }
    });
